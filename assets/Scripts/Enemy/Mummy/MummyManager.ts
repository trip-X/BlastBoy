import { _decorator, Collider2D, Color, director, instantiate, Prefab, UITransform, Vec3 } from 'cc'
...
import { MummyStateMachine } from './MummyStateMachine'
import { MummyBullet } from './MummyBullet'
...
const BULLET_PREFAB_URL = 'texture/Bullet/Mummy/Mummy'
const BULLET_SPEED = 5
const BASE_EXP = 6

@ccclass('MummyManager')
export class MummyManager extends EnemyManager {
  ...
  private lockedTargetPos: Vec3 = new Vec3()
  ...
  async init(params: IEntity, stats: IEnemyStats): Promise<void> {
    ...
    this.fsm = this.addComponent(MummyStateMachine)
    await this.fsm.init()
  ...
  private updateState(dt: number) {
    if (this.attackTimer > 0) {
      ...
      if (this.target === null) {
        ...
        this.lockedTargetPos.set(this.target.node.worldPosition)
  ...
  onAttackHit() {
    ...
    const bullet = bulletNode.getComponent(MummyBullet) || bulletNode.addComponent(MummyBullet)
    const startPos = this.node.worldPosition
    bullet.init(this.stats.doDamage(), BULLET_SPEED, startPos, this.lockedTargetPos, this.id)
  }
