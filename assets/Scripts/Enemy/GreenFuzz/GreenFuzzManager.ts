...
import { GreenFuzzStateMachine } from './GreenFuzzStateMachine'
...
const BODY_COLLIDER_WIDTH = BODY_WIDTH * 0.9
const BODY_COLLIDER_HEIGHT = BODY_HEIGHT * 0.9
const ATTACK_COOLDOWN = 3
const ATTACK_RADIUS = 0.15
const MOVE_SPEED = 0.45
const BASE_EXP = 4

@ccclass('GreenFuzzManager')
export class GreenFuzzManager extends EnemyManager {
  ...
  async init(params: IEntity, stats: IEnemyStats): Promise<void> {
    ...
    this.fsm = this.addComponent(GreenFuzzStateMachine)
    await this.fsm.init()
